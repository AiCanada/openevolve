"""Mild residual timewarp + screw/local — target dual-positive without evolve-overfit."""
import numpy as np
# EVOLVE-BLOCK-START
...
def predict_interior(ca_start, ca_end, fracs, mask):
    ...
    half = 13
    warp = 0.18  # mild (full 0.35 overfit in cand 18)
    ...
    alpha = np.clip(disp / (disp.max() + 1e-8), 0.0, 1.0) * m.astype(float)
    a2 = alpha.copy()
    for _ in range(2):
        a2[1:-1] = 0.25 * alpha[:-2] + 0.5 * alpha[1:-1] + 0.25 * alpha[2:]
        alpha = a2.copy() * m.astype(float)
    ...
    for ti, s in enumerate(fracs):
        ...
        se = s * s * (3.0 - 2.0 * s)
        t_res = (1.0 - warp * alpha) * s + (warp * alpha) * se
        screw[ti] = rigid + t_res[:, None] * residual
    ...
    # v3-like weights with slightly more lin for holdout
    return 0.15 * lin + 0.60 * screw + 0.15 * loc + 0.10 * lin  # = 0.25 lin + 0.60 screw + 0.15 loc
# EVOLVE-BLOCK-END
