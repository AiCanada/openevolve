...
def sample(rng: random.Random) -> dict:
    ...
    return {
        ...
        "w_dom": w[3],
        "w_mode": w[4],
        "local_half": rng.randint(12, 24),
        "half_s": rng.choice([3, 4, 5, 6]),
        "half_l": rng.choice([18, 20, 22, 24, 26]),
        "w_ms_small": ms[0],
        "w_ms_mid": ms[1],
        "w_ms_large": ms[2],
        "n_dom": rng.choice([1, 1, 2, 3]),
        "min_dom": rng.choice([6, 8, 10]),
        "use_chasles": rng.choice([0.0, 0.0, 0.0, 0.0, 0.3]),
        "timewarp": rng.choice([0.0, 0.0, 0.0, 0.0, 0.03, 0.06, 0.1]),
        "s_gate": rng.choice([0.0, 0.1, 0.15, 0.2, 0.25, 0.35]),
        "n_modes": rng.choice([0, 1, 1, 1, 2]),
        "mode_mid_boost": rng.choice([0.0, 0.05, 0.1, 0.113, 0.15]),
        "bond_strength": rng.choice([0.0, 0.0, 0.0, 0.0]),
    }
...
# Seeds centered on known dual_min + balanced champs
SEEDS = [
    # linear
    ...
    dict(ease=0, hermite=0, w_lin=0.122, w_screw=0.675, w_local=0.145, w_dom=0.0, w_mode=0.057,
         local_half=13, half_s=5, half_l=22, w_ms_small=0.0, w_ms_mid=1.0, w_ms_large=0.0,
         n_dom=1, min_dom=8, use_chasles=0.0, timewarp=0.0, s_gate=0.0, n_modes=1,
         mode_mid_boost=0.113, bond_strength=0),
    # v4 dual_min exact
    dict(ease=0, hermite=0.0, w_lin=0.10693815864018105, w_screw=0.6613103922323285,
         w_local=0.17530899683519152, w_dom=0.0, w_mode=0.05644245229130296,
         local_half=20, half_s=3, half_l=22, w_ms_small=0.13413454677601366,
         w_ms_mid=0.8556340414937043, w_ms_large=0.010231411729282176,
         n_dom=1, min_dom=9, use_chasles=0.0, timewarp=0.0, s_gate=0.14743748266883822,
         n_modes=1, mode_mid_boost=0.113, bond_strength=0),
    # dual wider local
    dict(ease=0, hermite=0, w_lin=0.10, w_screw=0.65, w_local=0.20, w_dom=0.0, w_mode=0.05,
         local_half=22, half_s=3, half_l=24, w_ms_small=0.12, w_ms_mid=0.85, w_ms_large=0.03,
         n_dom=1, min_dom=8, use_chasles=0.0, timewarp=0.0, s_gate=0.18, n_modes=1,
         mode_mid_boost=0.12, bond_strength=0),
    # dual stronger s_gate
    dict(ease=0, hermite=0, w_lin=0.12, w_screw=0.64, w_local=0.18, w_dom=0.0, w_mode=0.06,
         local_half=20, half_s=3, half_l=22, w_ms_small=0.15, w_ms_mid=0.82, w_ms_large=0.03,
         n_dom=1, min_dom=8, use_chasles=0.0, timewarp=0.0, s_gate=0.25, n_modes=1,
         mode_mid_boost=0.113, bond_strength=0),
    # dual more multiscale small
    dict(ease=0, hermite=0, w_lin=0.11, w_screw=0.64, w_local=0.19, w_dom=0.0, w_mode=0.06,
         local_half=18, half_s=4, half_l=22, w_ms_small=0.22, w_ms_mid=0.75, w_ms_large=0.03,
         n_dom=1, min_dom=8, use_chasles=0.0, timewarp=0.0, s_gate=0.15, n_modes=1,
         mode_mid_boost=0.13, bond_strength=0),
    # dual mild timewarp
    dict(ease=0, hermite=0, w_lin=0.11, w_screw=0.66, w_local=0.17, w_dom=0.0, w_mode=0.06,
         local_half=20, half_s=3, half_l=22, w_ms_small=0.13, w_ms_mid=0.85, w_ms_large=0.02,
         n_dom=1, min_dom=8, use_chasles=0.0, timewarp=0.04, s_gate=0.15, n_modes=1,
         mode_mid_boost=0.113, bond_strength=0),
    # mid Pareto-ish (more lin than dual)
    dict(ease=0, hermite=0, w_lin=0.20, w_screw=0.58, w_local=0.16, w_dom=0.0, w_mode=0.06,
         local_half=18, half_s=4, half_l=22, w_ms_small=0.10, w_ms_mid=0.88, w_ms_large=0.02,
         n_dom=1, min_dom=8, use_chasles=0.0, timewarp=0.0, s_gate=0.12, n_modes=1,
...
def main():
    ...
    log_path = OUT / "param_evolve_v5b_log.jsonl"
    ...
    try:
        ...
        pass
    print(f"=== param evolve v5b dual-primary: {args.iterations} iters device={args.device} ===", flush=True)
    ...
    if promo is not None:
        best_path = OUT / "best" / "best_program_v5b.py"
        best_path.parent.mkdir(parents=True, exist_ok=True)
        materialize(promo["params"], best_path)
        materialize(promo["params"], OUT / "best" / "best_program.py")
        materialize(promo["params"], EX / "candidates" / "30_v5b_dual_min.py")
        if best is not None:
            materialize(best["params"], OUT / "best" / "best_program_v5b_balanced.py")
        info = {
            "source": "param_evolve_v5b",
        ...
        (OUT / "param_evolve_v5b_summary.json").write_text(json.dumps(info, indent=2), encoding="utf-8")
        (OUT / "best" / "best_program_info.json").write_text(json.dumps(info, indent=2), encoding="utf-8")
        print("=== DONE v5b ===", flush=True)
