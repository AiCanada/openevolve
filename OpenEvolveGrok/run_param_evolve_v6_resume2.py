...
SEEDS = [
    {'ease': 0.14118708219571147, 'hermite': 0.049025862344506406, 'w_lin': 0.07694143693462112, 'w_screw': 0.4944542602533333, 'w_local': 0.3626800442490174, 'w_dom': 0.0, 'w_mode': 0.06592425856202144, 'local_half': 32, 'half_s': 2, 'half_l': 36, 'w_ms_small': 0.36534335836505244, 'w_ms_mid': 0.5930449798928207, 'w_ms_large': 0.041611661741126967, 'n_dom': 2, 'min_dom': 7, 'use_chasles': 0.004501034549267754, 'timewarp': 0.04575072558357838, 's_gate': 0.07549482534186869, 'n_modes': 1, 'mode_mid_boost': 0.24922512185593557, 'bond_strength': 0.0},
    {'ease': 0.02, 'hermite': 0.01476322545717388, 'w_lin': 0.046748850223038055, 'w_screw': 0.5933461299120433, 'w_local': 0.26499077963699225, 'w_dom': 0.028697600411392042, 'w_mode': 0.06621663981551656, 'local_half': 24, 'half_s': 2, 'half_l': 30, 'w_ms_small': 0.212563874000612, 'w_ms_mid': 0.7671126285544535, 'w_ms_large': 0.020323497443934527, 'n_dom': 1, 'min_dom': 9, 'use_chasles': 0.0, 'timewarp': 0.1, 's_gate': 0.18, 'n_modes': 1, 'mode_mid_boost': 0.22, 'bond_strength': 0.0},
    # neighborhood of dual
    dict(ease=0.05, hermite=0.0, w_lin=0.05, w_screw=0.48, w_local=0.38, w_dom=0.0, w_mode=0.09,
         local_half=30, half_s=2, half_l=34, w_ms_small=0.40, w_ms_mid=0.58, w_ms_large=0.02,
         n_dom=2, min_dom=7, use_chasles=0.0, timewarp=0.04, s_gate=0.10, n_modes=1,
         mode_mid_boost=0.28, bond_strength=0.0),
    dict(ease=0.0, hermite=0.0, w_lin=0.04, w_screw=0.50, w_local=0.36, w_dom=0.0, w_mode=0.10,
    ...
    dict(ease=0.03, hermite=0.0, w_lin=0.08, w_screw=0.60, w_local=0.24, w_dom=0.0, w_mode=0.08,
         local_half=24, half_s=2, half_l=28, w_ms_small=0.20, w_ms_mid=0.78, w_ms_large=0.02,
         n_dom=1, min_dom=8, use_chasles=0.0, timewarp=0.10, s_gate=0.17, n_modes=1,
         mode_mid_boost=0.20, bond_strength=0.0),
...
def main():
    ...
    ap.add_argument("--iterations", type=int, default=776)
    ap.add_argument("--seed", type=int, default=778)
    ...
    log_path = OUT / "param_evolve_v6_resume2_log.jsonl"
    ...
    try:
        ...
        pass
    print(f"=== param evolve v6 RESUME2 from 224 remaining 776: {args.iterations} iters device={args.device} ===", flush=True)
        ...
        for it in range(args.iterations):
            ...
            if (it + 1) % 50 == 0:
                ...
                if best_dual is not None:
                    ...
                    (OUT / "param_evolve_v6_resume2_checkpoint.json").write_text(
    ...
    if promo is not None:
        ...
        (OUT / "param_evolve_v6_resume2_summary.json").write_text(json.dumps(info, indent=2), encoding="utf-8")
