...
def _score(program_path, n_windows=None, split="evolve", timeout=120):
    ...
    def _paired_imp(wm, wb, floor):
        """Median absolute improvement / max(baseline_median, floor).

        Floors prevent explosion when baseline COM/angle is near zero (linear COM
        error is ~0.006 Å); folding uses floor=0.5 Å so Kabsch RMSD stays relative.
        """
        ...
        denom = max(abs(med_b), float(floor))
        imp = (med_b - med_m) / denom
        # soft clip so one bad component cannot dominate MAP-Elites
        imp = float(np.clip(imp, -1.0, 2.0))
        return imp, med_m, med_b, float((wm < wb).mean())

    # floors: COM/rg ~0.01, angle ~0.01 rad, fold RMSD ~0.5 Å
    t_imp, t_m, t_b, t_wr = _paired_imp(win_t_m, win_t_b, floor=0.01)
    r_imp, r_m, r_b, r_wr = _paired_imp(win_r_m, win_r_b, floor=0.01)
    f_imp, f_m, f_b, f_wr = _paired_imp(win_f_m, win_f_b, floor=0.5)
    ...
    out = {
        ...
        "improvement_pct": float(f_imp * 100.0),  # fold primary for backward compat
        "win_rate": float(f_wr),
        "model_rmsd": float(f_m),       # folding = Kabsch RMSD
